...
def convert_hex_to_decimal(input_file, output_file):
    ...
    # Convert each hexadecimal number to decimal, invert the bytes, and save every other number
    decimal_numbers = [invert_bytes(int(hex_num, 16)) for index, hex_num in enumerate(hex_numbers)]
...
# Call the function to convert hexadecimal to decimal, invert the bytes, and save every other number
