...
LOG_MODULE_REGISTER(esb_prx,LOG_LEVEL_DBG);
...
{ 
	...
	case ESB_EVENT_RX_RECEIVED:
		if (esb_read_rx_payload(&rx_payload) == 0) {
			...
			// 	rx_payload.data[7]);
			//data is coming in very quickly, print every 100th sample?
			print_count ++;
			...
			//if(print_count % 1000 == 0){
			 	//LOG_ERR("RX count: %d\n", print_count);
				//LOG_DBG("RX count: %d", print_count);
			 	LOG_DBG("Packet received, len %d : 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x", rx_payload.length, rx_payload.data[5], rx_payload.data[4], rx_payload.data[3], rx_payload.data[2], rx_payload.data[1], rx_payload.data[0]);
			// 	print_count = 0;
			//print_next = true;
			//}
			//if(print_count % 1000 == 0)
			 //	LOG_DBG("%d", print_count);
...
//converts rx payload into count value to be printed
